...
const DeliverySchema = Type.Union([Type.Literal('followUp'), Type.Literal('steer')], {
...
function deliveryOptions(message: ShepherdMessageEnvelope): {
  deliverAs: ShepherdDelivery;
  triggerTurn: boolean;
} {
  return {
    ...
    triggerTurn:
      message.kind === 'reply' || message.kind === 'task' || message.expectsReply === true,
  };
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description:
      'Send an asynchronous message to the Shepherd or another registered agent without waiting for a reply. For a peer, target MUST be the exact opaque agent id returned by shepherd_spawn; copy it verbatim. Never use an agent definition name such as "planner", a display label, a Herdr pane id, or a placeholder such as "<planner agent ID>"—the call will be rejected.',
    promptSnippet:
      'send an asynchronous message using the exact opaque agent id returned by shepherd_spawn',
    promptGuidelines: [
    ...
    parameters: Type.Object({
      target: Type.String({
        description:
          'Use "parent"/"shepherd" for the parent, or the exact opaque peer agent id returned by shepherd_spawn. Agent names (for example "planner"), labels, pane ids, and placeholders are rejected.',
      }),
      message: Type.String({ description: 'Non-empty message content.' }),
      taskId: Type.Optional(
        Type.String({
          description: 'Active delegated task id, when this message belongs to a task.',
        })
      ),
    ...
    async execute(_toolCallId, args) {
      ...
      if (!active)
        return unavailableToolResult(
          'shepherd_message',
          args,
          brokerError ?? 'Broker unavailable.'
        );
      const referencedTaskId = args.replyTo ? receivedMessageTaskIds.get(args.replyTo) : undefined;
      if (
        args.replyTo &&
        receivedMessageTaskIds.has(args.replyTo) &&
        args.taskId !== undefined &&
        args.taskId !== referencedTaskId
      ) {
        return childResult(
          'shepherd_message',
          args,
          ...
          {
            accepted: false,
            error: `Reply ${args.replyTo} belongs to task ${referencedTaskId ?? 'none'}; use that task id, not ${args.taskId}.`,
          },
          {
            code: 'reply_task_mismatch',
            replyTo: args.replyTo,
            expectedTaskId: referencedTaskId ?? null,
            providedTaskId: args.taskId,
          },
          1
        );
      }
      ...
      try {
        ...
        if (
          ...
          args.replyTo &&
          !['shepherd', 'parent', active.parentId].includes(args.target)
        ) {
        ...
        if (accepted.delivery === 'queued' && args.expectsReply === true) {
          ...
          const mirrored = createEnvelope(
            { sessionId: active.sessionId, brokerId: active.brokerId, senderId: active.agentId },
            {
              ...
              requestOpen: true,
              requestTargetId: ['shepherd', 'parent'].includes(args.target)
                ? active.parentId
                : args.target,
              summary: args.message,
              delivery: 'followUp',
        ...
        return childResult(
          ...
          {
            code: (error as any)?.code ?? 'message_rejected',
            error: String((error as any)?.message ?? error),
          },
          1
        );
  ...
  pi.registerTool({
    ...
    description:
      'Explicitly complete, block, or fail the current delegated Shepherd task. This is the only signal that tells the parent a tracked task has finished.',
    ...
    async execute(_toolCallId, args) {
      ...
      try {
        ...
        return childResult(
          ...
          { accepted: false, error: String((error as any)?.message ?? error) },
          {
            code: (error as any)?.code ?? 'completion_rejected',
            error: String((error as any)?.message ?? error),
          },
          1
        );
