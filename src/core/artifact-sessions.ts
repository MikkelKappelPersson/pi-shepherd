...
export type ArtifactStatus =
  'pending' | 'running' | 'completed' | 'failed' | 'timed-out' | 'cancelled';
  ...
  while (true) {
    try {
      ...
      if (Date.now() - started > LOCK_TIMEOUT_MS)
        throw new Error(`Timed out acquiring filesystem lock: ${lockPath}`);
      sleep(LOCK_RETRY_MS);
    }
  }
}
...
function validateParentIdentity(identity: unknown): string {
  if (typeof identity !== 'string' || !identity)
    throw new TypeError('A stable parent pi session identity is required');
  if (/[\u0000-\u001f\u007f]/.test(identity))
    throw new Error('Parent pi session identity contains control characters');
  return identity;
}
...
export function slugifySessionName(name: string): string {
  ...
  if (
    ...
    throw new Error(
      'Session name must not contain control characters, path separators, or be an absolute path'
    );
  }
...
export function deriveSessionName(task?: string): string {
  ...
  try {
    return slugifySessionName(words);
  } catch {
    return 'delegation';
  }
}
  ...
  try {
    ...
    if (value.artifactSessionVersion === 2) {
      ...
      if (typeof identity !== 'string' || !identity || /[\u0000-\u001f\u007f]/.test(identity))
        return undefined;
      if (value.parentPiSession && path.resolve(value.parentPiSession.projectRoot) !== projectRoot)
        return undefined;
    }
    const artifacts = value.artifacts.map(artifact => {
      if (
        ...
        !Number.isInteger(artifact.ordinal)
      )
...
function collisionSlug(root: string, slug: string, sessionName: string): string {
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    if (
      ...
      (() => {
        try {
          return slugifySessionName(record.sessionName) === slug;
        } catch {
          return false;
        }
      })()
...
function renderMoc(record: SessionRecord): string {
  const modes = record.modes.length
    ? record.modes.join(', ')
    : record.artifacts
        .map(a => a.mode)
        ...
        .join(', ');
  const binding =
    record.artifactSessionVersion === 2 &&
    (record.parentPiSession?.identity ?? record.parentPiSessionId)
  ...
  const artifacts = record.artifacts.length
    ? record.artifacts
        .map(
          (a, i) =>
            `${i + 1}. [${displayName(a.agent)} ${String(a.ordinal).padStart(2, '0')}](./${a.fileName}) — ${a.status}`
        )
        .join('\n')
    : '_No notes reserved yet._';
  const flow = record.artifacts.length
    ? record.artifacts.map(a => `[${a.fileName.replace(/\.md$/, '')}](./${a.fileName})`).join(' → ')
    : '_No flow yet._';
...
function writeMoc(record: SessionRecord): void {
  atomicWrite(path.join(record.sessionPath, 'shepherd.md'), renderMoc(record));
}
function updateRecordAndMoc(record: SessionRecord): void {
  record.updatedAt = now();
  writeRecord(record.sessionPath, record);
  writeMoc(record);
}
  ...
  try {
    if (resumeExisting) {
      for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
        if (!entry.isDirectory() || !isSessionDirectory(entry.name)) continue;
        const record = readRecord(path.join(root, entry.name));
        if (!record || record.projectRoot !== projectRoot || record.sessionName !== requested)
          continue;
        ...
        try {
          ...
          record.status = ['completed', 'failed', 'timed-out', 'cancelled'].includes(record.status)
            ? 'running'
            : record.status;
    ...
    const record: SessionRecord = {
      ...
      updatedAt: startedAt,
      status: 'running',
      modes: options.mode ? [options.mode] : [],
      artifacts: [],
    };
...
function loadSession(session: ShepherdSession): SessionRecord {
  ...
  if (record.sessionPath !== sessionPath)
    throw new Error('Session path does not match durable metadata');
...
function syncHeldSession(session: ShepherdSession, record: SessionRecord): void {
  ...
  for (const key of Object.keys(record) as Array<keyof SessionRecord>)
    (session as any)[key] = (record as any)[key];
}
...
function initializeArtifact(a: ArtifactReservation, session: SessionRecord): void {
  atomicWrite(a.filePath, artifactFrontmatter(a, session) + '\n');
}

/** Reserve all artifacts in input order, before any corresponding child starts. */
export function reserveArtifacts(
  ...
  try {
    ...
    for (const item of planned) {
      if (!item || typeof item.agent !== 'string' || !item.agent.trim())
        throw new Error('Artifact agent name is required');
      ...
      let suffix = 2;
      while (
        record.artifacts.some(a => a.fileName === fileName) ||
        fs.existsSync(path.join(record.sessionPath, fileName))
      )
        fileName = `${base}-${suffix++}-${String(ordinal).padStart(2, '0')}.md`;
      const reservedAt = now();
      const artifact: ArtifactReservation = {
        id: `${record.directoryName}/${fileName}`,
        agent: item.agent,
        agentSlug: base,
        ordinal,
        fileName,
        filePath: path.join(record.sessionPath, fileName),
        relativePath: path.relative(record.projectRoot, path.join(record.sessionPath, fileName)),
        mode: item.mode,
        task: item.task,
        status: 'pending',
        reservedAt,
    ...
    if (planned.some(p => p.mode && !record.modes.includes(p.mode)))
      record.modes = [
        ...record.modes,
        ...planned
          .map(p => p.mode)
          .filter((m, i, all) => !record.modes.includes(m) && all.indexOf(m) === i),
      ];
    updateRecordAndMoc(record);
    return result;
  } finally {
...
export function markArtifactStarted(
  session: ShepherdSession,
...
export function markArtifactStatus(
  session: ShepherdSession,
  artifact: ArtifactReservation,
  status: ArtifactStatus,
  metadata: Record<string, unknown> = {}
...
function updateArtifact(
  ...
  try {
    ...
    if (['completed', 'failed', 'timed-out', 'cancelled'].includes(status))
    ...
    atomicWrite(
      stored.filePath,
      artifactFrontmatter(stored, record, metadata) + body.trimStart() + '\n'
    );
    updateRecordAndMoc(record);
...
export function finalizeArtifact(
  ...
  if (!['completed', 'failed', 'timed-out', 'cancelled'].includes(result.status))
    throw new Error('Final artifact status must be terminal');
  const release = lock(path.join(session.sessionPath, '.session.lock'));
  try {
    const record = loadSession(session);
    const stored = record.artifacts.find(
    ...
    Object.assign(artifact, stored);
    const existing = fs.existsSync(stored.filePath)
      ? fs.readFileSync(stored.filePath, 'utf8')
      : artifactFrontmatter(stored, record);
    const body = existing
      .replace(/^---[\s\S]*?---\s*/, '')
      .replace(/\n<!-- Shepherd orchestration -->[\s\S]*$/, '')
      .trimEnd();
    ...
    atomicWrite(
      stored.filePath,
      artifactFrontmatter(stored, record, result.metadata ?? {}) + body + section
    );
    updateRecordAndMoc(record);
    syncHeldSession(session, record);
  } finally {
...
export function updateSessionMoc(
  session: ShepherdSession,
  update: { mode?: SessionMode; status?: string } = {}
): ShepherdSession {
  const release = lock(path.join(session.sessionPath, '.session.lock'));
  try {
 ...
 */
export function resolveOrCreateParentArtifactSession(
  options: ParentArtifactSessionOptions
): ArtifactSession {
  const parentPiSessionId = validateParentIdentity(
    options.parentPiSessionIdentity ?? options.parentPiSessionId
  );
  const projectRoot = path.resolve(options.projectRoot);
  ...
  try {
    for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
      ...
      try {
        ...
        record.resumed = true;
        if (['completed', 'failed', 'timed-out', 'cancelled'].includes(record.status))
