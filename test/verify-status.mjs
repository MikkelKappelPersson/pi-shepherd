...
await withFakeDateNow(0, async clock => {
  ...
  assert.equal(
    idleStatus.task.pendingRequestMessageId,
    'msg-status-1',
    'pending request id is included'
  );
  assert.equal(
    idleStatus.task.waitingRecipient,
    'planner: status-planner',
    'waiting recipient is included'
  );
  ...
  assert.equal(
    workerStatus.task.pendingRequestMessageId,
    undefined,
    'no pending request for a running task'
  );
...
console.log(
  'PASS status includes task id, waiting age, pending request, recipient, and stale flag'
);
