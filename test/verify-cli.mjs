...
import {
  parseShepherdCli,
  formatShepherdCommand,
  statusHandleTarget,
  tokenizeCli,
} from '../src/extension/cli.ts';
...
check(
  parsed(['agents', '--scope', 'project']).args.agentScope === 'project',
  'agents --scope: flag scope'
);

const okHerd = parsed(['herd']);
check(
  !isError(okHerd) && JSON.stringify(okHerd.args) === JSON.stringify({ action: 'herd' }),
  'herd: bare'
);

const okSpawn = parsed(['spawn', 'worker']);
check(
  !isError(okSpawn) && okSpawn.args.agent === 'worker' && okSpawn.action === 'spawn',
  'spawn worker: agent name'
);

const okSpawnFlags = parsed(['spawn', 'worker', '--placement=pane_down', '--label=down-split']);
check(
  okSpawnFlags.args.placement === 'pane_down' &&
    okSpawnFlags.args.label === 'down-split' &&
...
check(
  okSpawnSpace.args.cwd === '/tmp/x' &&
    !('model' in okSpawnSpace.args) &&
...
function okReadChecks() {
  ...
  check(
    okRead.args.name === 'pane123' && okRead.args.lines === 10 && okRead.args.source === 'recent',
    'read: name + integer lines + source enum'
  );
  const okReadSpace = parsed(['read', 'p5', '--lines', '7', '--source', 'visible']);
  check(
    okReadSpace.args.lines === 7 && okReadSpace.args.source === 'visible',
    'read: space-separated options'
  );
}

const okStatus = parsed(['status', 'worker']);
check(
  !isError(okStatus) && okStatus.args.id === 'worker',
...
for (const [toks, hint] of shouldFail) {
  ...
  check(
    pass,
    `rejects ${JSON.stringify(toks)}${p.error ? '' : '  (parsed: ' + JSON.stringify(p.args) + ')'}`
  );
}
...
function roundTrip(toks) {
  ...
  return p.action === reparsed.action &&
    JSON.stringify(drop(p.args)) === JSON.stringify(drop(reparsed.args))
    ? null
...
check(
  preview.verb === 'spawn' &&
    preview.rest.includes('scout') &&
    preview.rest.includes('--cwd="') &&
    !preview.rest.includes('--scope') &&
    !preview.rest.includes('--direction'),
