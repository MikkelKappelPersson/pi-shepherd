...
const okSpawnFlags = parsed([
	'spawn', 'worker', '--placement=pane', '--direction=down', '--scope=project', '--omit-system-prompt',
]);
check(
	okSpawnFlags.args.placement === 'pane' &&
		okSpawnFlags.args.direction === 'down' &&
		okSpawnFlags.args.agentScope === 'project' &&
		okSpawnFlags.args.omitSystemPrompt === true,
	'spawn: flag=value and bare boolean options',
);

const okSpawnSpace = parsed(['spawn', 'scout', '--cwd', '/tmp/x', '--model', 'openai/gpt-x', '--direction', 'down']);
check(
	okSpawnSpace.args.cwd === '/tmp/x' && okSpawnSpace.args.model === 'openai/gpt-x' && okSpawnSpace.args.direction === 'down',
	'spawn: space-separated flag values',
);

const okBoolFlag = parsed(['spawn', 'w', '--confirm-project-agents', 'false']);
check(okBoolFlag.args.confirmProjectAgents === false, 'spawn: --confirm-project-agents false coerced to boolean');
...
function roundTrip(toks) {
	...
	const drop = (o) => {
		const copy = { ...o };
		// omitSystemPrompt is a bare flag; a rendered `false` value is omitted
		// (the schema default applies), so compare with the key dropped.
		delete copy.omitSystemPrompt;
...
for (const toks of [
	...
	['herd'],
	['spawn', 'worker'],
	['spawn', 'worker', '--placement=pane', '--direction=down', '--scope=project'],
	['spawn', 'worker', '--confirm-project-agents=false'],
