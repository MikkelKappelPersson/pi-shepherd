...
try {
  ...
  if (!enabledPrompt.includes('UNIQUE_AGENT_BODY_ENABLED_1a2b3c4d'))
    failures.push('enabled agent body remains present');
  if (!disabledPrompt.includes('UNIQUE_AGENT_BODY_DISABLED_5e6f7a8b'))
    failures.push('disabled agent body remains present');
  if (enabled.cwd !== project || disabled.cwd !== project)
    failures.push('child cwd remains the requested project');
