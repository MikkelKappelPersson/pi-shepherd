...
await check('same exact name resumes rather than duplicates', () => {
  const resumed = createOrResumeSession({
    projectRoot: project,
    sessionName: 'fix-oauth-login',
    mode: 'parallel',
  });
  ...
  assert.equal(
    fs.readdirSync(path.join(project, '.shepherd', 'sessions')).filter(x => x.match(/^0001-/))
      .length,
    1
  );
});
await check('same slug different names remain separate', () => {
  const other = createOrResumeSession({
    projectRoot: project,
    sessionName: 'fix oauth login',
    mode: 'chain',
  });
...
await check('parent pi binding reuses one session across calls', () => {
  const firstParent = resolveOrCreateParentArtifactSession({
    projectRoot: project,
    parentPiSessionId: 'parent-pi-a',
    parentSessionFile: '/tmp/parent-a.jsonl',
  });
  const secondParent = resolveOrCreateParentArtifactSession({
    projectRoot: project,
    parentPiSessionId: 'parent-pi-a',
    parentSessionFile: '/tmp/parent-a-resumed.jsonl',
  });
  const otherParent = resolveOrCreateParentArtifactSession({
    projectRoot: project,
    parentPiSessionId: 'parent-pi-b',
  });
  assert.equal(firstParent.sessionPath, secondParent.sessionPath);
  assert.notEqual(firstParent.sessionPath, otherParent.sessionPath);
  assert.equal(firstParent.artifactSessionVersion, 2);
  assert.equal(firstParent.parentPiSession?.identity, 'parent-pi-a');
  assert.equal(
    JSON.parse(fs.readFileSync(firstParent.sessionMetadataPath, 'utf8')).parentPiSession
      .sessionFile,
    '/tmp/parent-a-resumed.jsonl'
  );
});
await check('legacy unbound sessions are not implicitly claimed', () => {
  const legacy = createOrResumeSession({ projectRoot: project, sessionName: 'legacy-unbound' });
  const bound = resolveOrCreateParentArtifactSession({
    projectRoot: project,
    parentPiSessionId: 'parent-pi-legacy',
  });
  assert.notEqual(bound.sessionPath, legacy.sessionPath);
  assert.equal(
    JSON.parse(fs.readFileSync(legacy.sessionMetadataPath, 'utf8')).artifactSessionVersion,
    undefined
  );
});
...
await check('reserve per-agent artifacts before execution with relative links', () => {
  ...
  assert.deepEqual(
    artifacts.slice(0, 2).map(a => a.fileName),
    ['scout-01.md', 'scout-02.md']
  );
  assert.equal(new Set(artifacts.map(a => a.fileName)).size, artifacts.length);
  assert.ok(
    artifacts.every(a => !path.isAbsolute(a.relativePath) && a.relativePath.includes('.shepherd'))
  );
  ...
  assert.equal(
    fs.readdirSync(first.sessionPath).some(name => name.includes('.tmp-')),
    false
  );
});
await check('lifecycle metadata and output/error persistence', () => {
  ...
  const metadata = JSON.parse(fs.readFileSync(first.sessionMetadataPath, 'utf8'));
  assert.deepEqual(
    metadata.artifacts.map(a => a.status),
  ...
  assert.deepEqual(
    artifacts.map(a => a.status),
    ['completed', 'failed', 'timed-out', 'cancelled'],
    'in-memory reservations mirror canonical status'
  );
  ...
  assert.deepEqual(
    first.artifacts.map(a => a.status),
    ['completed', 'failed', 'timed-out', 'cancelled'],
    'held session artifacts array mirrors canonical state'
  );
...
await check('concurrent numbering uses unique ordinals', async () => {
  ...
  const dirs = fs
    .readdirSync(path.join(concurrentRoot, '.shepherd', 'sessions'))
    .filter(x => /^\d{4,}-/.test(x));
  assert.equal(dirs.length, 8);
  assert.equal(new Set(dirs.map(x => x.split('-')[0])).size, 8);
  assert.equal(
    fs.existsSync(path.join(concurrentRoot, '.shepherd', 'sessions', '.alloc', '0001')),
    true
  );
});
