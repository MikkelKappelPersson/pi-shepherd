...
await check("parent pi binding reuses one session across calls", () => {
  const firstParent = resolveOrCreateParentArtifactSession({ projectRoot: project, parentPiSessionId: "parent-pi-a", parentSessionFile: "/tmp/parent-a.jsonl" });
  const secondParent = resolveOrCreateParentArtifactSession({ projectRoot: project, parentPiSessionId: "parent-pi-a", parentSessionFile: "/tmp/parent-a-resumed.jsonl" });
  const otherParent = resolveOrCreateParentArtifactSession({ projectRoot: project, parentPiSessionId: "parent-pi-b" });
  ...
  assert.equal(JSON.parse(fs.readFileSync(firstParent.sessionMetadataPath, "utf8")).parentPiSession.sessionFile, "/tmp/parent-a-resumed.jsonl");
});
await check("legacy unbound sessions are not implicitly claimed", () => {
  ...
  const bound = resolveOrCreateParentArtifactSession({ projectRoot: project, parentPiSessionId: "parent-pi-legacy" });
  assert.notEqual(bound.sessionPath, legacy.sessionPath);
  assert.equal(JSON.parse(fs.readFileSync(legacy.sessionMetadataPath, "utf8")).artifactSessionVersion, undefined);
});
