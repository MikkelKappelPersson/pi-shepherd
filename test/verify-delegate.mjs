...
assert.deepEqual(Object.keys(delegateTool.parameters.properties).sort(), [
  'target',
  'task',
  'timeout',
]);
...
`
  ...
  let agent;
  try {
    agent = lifecycleRegistry.registerAgent({
      agent: 'scout',
      label: 'delegate test',
      paneId: 'pane-delegate-test',
    });
    ...
    assert.ok(
      ...
      'delegation returns without waiting for task completion'
    );
    ...
    console.log(
      'PASS delegation creates a running task and queues a task envelope without waiting'
    );
    ...
    const failedAgent = lifecycleRegistry.registerAgent({
      agent: 'worker',
    ...
    const failedTask = lifecycleRegistry
      .allTasks()
      .find(record => record.agentId === failedAgent.id);
    ...
    await assert.rejects(
      () =>
        delegateAgent(closedAgent, 'This must not be submitted.', {
          sessionId: 'delegate-test-session',
        }),
...
await assert.rejects(() => delegateAgent('not-an-agent', ''), /must not be empty/);
