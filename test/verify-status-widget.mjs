 ...
 * agent whose pane is in the created-panes registry, and HERDR_ENV=1 forces
 * Herdr availability so workingSubagents() -> listHerdrAgents() -> shim.
 *
...
// Sandboxed created-panes registry: one working shepherd agent.
const PANE_ID = "test-pane-1";
writeFileSync(
	path.join(agentDir, "pi-shepherd", "created-panes.json"),
	JSON.stringify(
		[{ paneId: PANE_ID, tabId: "test-tab-1", name: "worker", cwd: otherCwd, createdAt: Date.now() }],
		null,
		2,
	),
);
...
// the agent list (`herdr agent list`).
...
case "$1" in
  ...
  agent)
    cat <<'EOF'
{"result":{"agents":[{"agent":"pi","agent_status":"working","pane_id":"${PANE_ID}","tab_id":"test-tab-1","workspace_id":"test-ws-1","foreground_cwd":"${otherCwd}","focused":false,"terminal_title":"pi"}]}}
EOF
    exit 0 ;;
esac
exit 0
...
assert.match(p1[0], /1 working/, "top border shows the worker count");
