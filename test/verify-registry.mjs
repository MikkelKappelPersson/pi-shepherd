...
assert(
  preparedLegacy.id === 'agent-1' && !('handle' in preparedLegacy),
  'legacy handle is migrated to id'
);
...
assert(
  JSON.stringify(preparedArray.id) === JSON.stringify(['prompt-1', 'prompt-2']),
  'stringified id array is normalized before validation'
);
assert(
  formatIdForModel('shepherd-agent-1') === 'shepherd-agent-1',
  'model-facing id text remains copyable'
);
...
assert(
  !('agentScope' in normalizedOptions) &&
    !('direction' in normalizedOptions) &&
    !('confirmProjectAgents' in normalizedOptions) &&
    !('omitSystemPrompt' in normalizedOptions),
  'settings-owned spawn options are discarded'
);
...
assert(
  JSON.parse(JSON.stringify(agent)).id === agent.id,
  'stable internal agent handle serialization'
);
assert(registry.getAgent(agent.id).handle.id === agent.id, 'agent id resolves in registry');
try {
  registry.getAgent(JSON.stringify(agent));
  assert(false, 'quoted agent object rejected');
} catch (e) {
...
assert(
  JSON.parse(JSON.stringify(prompt)).agentId === agent.id,
  'stable internal prompt handle serialization'
);
assert(registry.getPrompt(prompt.id).handle.id === prompt.id, 'prompt id resolves in registry');
try {
  registry.getPrompt(JSON.stringify(prompt));
  assert(false, 'quoted prompt object rejected');
} catch (e) {
...
try {
  registry.createPrompt(agent);
  assert(false, 'duplicate active prompt rejected');
} catch (e) {
  assert(e.code === 'active_prompt', 'duplicate active prompt rejected');
}
...
assert(
  (await registry.wait(second)).returnCode === 130,
  'cancelled prompt includes return code 130'
);
assert(registry.status(agent).state === 'closed', 'close marks agent closed');
try {
  registry.getAgent({ id: 'foreign' });
  assert(false, 'unknown handle rejected');
} catch (e) {
  assert(e.code === 'unknown_handle', 'unknown handle rejected');
}
